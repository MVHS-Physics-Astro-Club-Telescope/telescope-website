...
The home page renders an idealised version of the instrument, drawn the way a finished, machined telescope looks under one studio spotlight. It is not a CAD export: `src/components/telescope/IdealTelescope.tsx` builds it from primitives using the dimensions in `spec.ts`, which follow the real build (254 mm f/4.48, 6-pole truss, 360 mm mirror box, 508 mm ground board). The lower assembly is brushed aluminium plate (cradle-topped rocker sides with a lightening hole, hand-cutout front plate, laminate-rimmed bearing arcs, a dark-anodised mirror box flocked black inside) with socket-head fasteners; the control box on the rear of the rocker holds a Raspberry Pi 4, two DM542T stepper drives and a terminal strip behind a smoked lid that dissolves when the camera dives in. The Pi is not a modelled approximation: `PiBoard.tsx` maps a rectified photograph of a real Pi 4 (Wikimedia Commons, Laserlicht, CC BY-SA 4.0; derived maps and licence in `public/textures/pi/`) onto the PCB outline and onto the top of every package, with package heights from the Raspberry Pi mechanical drawing, so the silkscreen, traces, pads and chip markings are the real ones. The drives are modelled from the DM542T's dimensions with a generated settings label (`public/textures/dm542_label.jpg`). The brushed normal/roughness maps are generated (`public/textures`); reflections come from a CC0 photo-studio HDRI from Poly Haven (`public/hdr`).
