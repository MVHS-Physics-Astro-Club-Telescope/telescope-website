...
## [2026-09-16][dns-outage-delegation-not-records] mvhsastro.org outage was a delegation change, not deleted records — and we don't own the domain
- **Phase**: site outage — `www.mvhsastro.org` NXDOMAIN, apex NODATA
- **Mistake**: Two wrong turns before the real answer. (1) Never checked **who owns the domain** before trying to get into the registrar — the vault has said since 2026-04-15 that friend **Isaac** registered it and owns registrar/DNS while Eeshan owns only Vercel, so three password resets were burned on an account that was never his. (2) Concluded the records had been *deleted* by a registrar platform migration, on the strength of a banner on the registrar's own front page. They were never deleted.
- **Root cause**: The Cloudflare zone in Isaac's account is **completely intact** — apex and `www` CNAME to `4614797125fa23c9.vercel-dns-017.com`, both `_vercel` verification TXTs, full Zoho mail set, all live and answering when queried directly at `jacob.ns.cloudflare.com` / `sky.ns.cloudflare.com`. The domain's **delegation at the registry** was changed to `ns1/ns2.hosting.businessidentity.llc` (registrar now DomainRegistry.com LLC, registry `Updated Date: 2026-09-10`). Nothing was deleted; the zone simply stopped being consulted.
- **Fix**: One field, at the registrar, by Isaac (`isaacgallienne@icloud.com`) — set nameservers to `jacob.ns.cloudflare.com` and `sky.ns.cloudflare.com`. No DS record is published (`dig mvhsastro.org DS` empty), so no DNSSEC breakage. Verified end to end: resolving through Cloudflare's own answers returns `www` 200 / valid TLS / `<title>MV Astronomy</title>` and apex 307 to www. Caveat: the live zone routes mail to Google, the Cloudflare zone to Zoho — the switch moves @mvhsastro.org mail to Zoho.
- **Prevention**: For any "site can't be reached", separate the two failure modes immediately — **records gone** vs **delegation changed**. Compare `dig NS <domain>` against the DNS provider you believe owns the zone, and query the believed nameservers *directly* (`dig @<expected-ns> <host>`): if they answer correctly, the zone is fine and only the delegation is wrong. That one query would have skipped the entire registrar detour. And check ownership in the vault (`grep -ri <domain> ~/obsidian-vault/daily/`) before assuming you can fix a domain at all — `whois` names the registrar, never the account holder.

## [2026-09-17][mvhsastro-contact-email] contact@mvhsastro.org: Apple Business email was a dead end, Cloudflare Email Routing was the fix
- **Phase**: giving the club a real address on its own domain
- **Mistake**: Nearly built on the mail stack that was already in DNS. Isaac had pointed the domain at **Apple Business email** (`MX mx01/mx02.mail.business.icloud.com`, `v=spf1 include:businessmail.apple ~all`, `sig1._domainkey` CNAME) and mailed a Managed Apple Account (`eeshan@mvhsastro.org`) with a temp password — so the obvious move was to sign in and add `contact@`.
- **Root cause**: Two hard blocks, neither visible from DNS. (1) `business.apple.com` answers **"You must be 18 or older to use Apple Business"** — a high-school student cannot administer the org at all, so only Isaac can create addresses. (2) Nothing was actually provisioned: an SMTP RCPT probe at Apple's own MX returned `550 5.1.1 <contact@mvhsastro.org>: user does not exist` **and the same for `eeshan@mvhsastro.org`**. Apple's docs for that product also describe no shared mailboxes, aliases, or forwarding.
- **Fix**: Cloudflare Email Routing on the same zone (free, Eeshan is admin on Isaac's account): destination `mvhsphysicsastroclub@gmail.com` verified, rule `contact@mvhsastro.org → club Gmail` plus catch-all, Apple's two MX deleted, SPF replaced with `v=spf1 include:_spf.mx.cloudflare.net ~all`, `cf2024-1._domainkey` DKIM added, and a new `_dmarc` TXT (`v=DMARC1; p=none; rua=mailto:contact@mvhsastro.org`). Verified by sending from the personal Gmail to `contact@` — it landed in the club **inbox**, not spam.
- **Prevention**: Before designing around a mail provider that DNS says is live, prove a mailbox exists: `python3 -c` an SMTP `MAIL FROM`/`RCPT TO` probe against the MX and read the code. `550 user does not exist` for *every* address means the service is configured but not provisioned. Two Cloudflare specifics: "Add missing records" refuses while any third-party MX is present ("Existing non-Cloudflare MX records conflict with Email Routing"), and it keeps reporting the SPF TXT as Missing unless the record matches its string exactly — set it to Cloudflare's value first, then click, or you end up with two SPF records and a permerror.
